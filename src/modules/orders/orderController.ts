...
export const orderController = {
    ...
    saveOrder: async (req: Request, res: Response) => {
        try {
            ...
            const result: any = await orderRabbitMqClient.produce(CourseData, operation);
            ...
            const userResult: any = await userRabbitMqClient.produce(userCourseData, userOperation);
            ...
            const courseResult: any = await courseRabbitMqClient.produce(courseUpdateData, courseOperation);
        ...
        const chatData = { userId, courseId };  // Send userId and courseId to the Chat service
        const chatResult: any = await chatRabbitMqClient.produce(chatData, chatOperation);
        ...
        return res.json({
            ...
            chatUpdate: chatResult,  // Include chat update result
        });
        } catch (error) {
            console.log("Error in making payment")
        }
