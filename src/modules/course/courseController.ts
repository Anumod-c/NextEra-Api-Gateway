...
export const courseController = {
    ...
    singleCourse: async (req: Request, res: Response) => {
        try {
            ...
            if(userId){
                const userResult: any = await userRabbitMqClient.produce(userId, userOperation);
                console.log('userreslt', userResult)
                const hasPurchased = userResult.user.purchasedCourses.includes(courseId);
    ...
    fetchMyCourses:async(req:Request,res:Response)=>{
        try {
            ...
            const result :any= await courseRabbitMqClient.produce(enrolledCourses,operation)
